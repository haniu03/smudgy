...
state-disabled = Вимкнути
state-enabled = Увімкнути
...
preferences-bold-is-bright = Використовувати яскраві кольори для жирного тексту термінала
preferences-bold-is-bright-help = Коли цю опцію ввімкнено, текст із жирним накресленням SGR також використовує яскраву палітру ANSI. Явні коди яскравих кольорів залишаються яскравими незалежно від цього налаштування.
...
preferences-link-tooltip-delay-help = Час, протягом якого вказівник має залишатися над посиланням, перш ніж з’явиться підказка. 0 показує її одразу; максимум — 60000. Натисніть Enter, щоб застосувати.
...
profile-plaintext-notice = Текст автоматичного входу зберігається на цьому пристрої як звичайний текст. Використовуйте $PASSWORD, щоб безпечно підставити пароль.
profile-keychain-notice = Текст автоматичного входу зберігається як звичайний текст; значення $PASSWORD зберігається у зв'язці ключів системи.
...
mapper-copy-boundary-help = За замовчуванням вони пропускаються. Якщо їх додати, кожне стає висячим одностороннім з'єднанням без збережених точок маршруту.
...
mapper-save-local = На цьому пристрої — лише локально, без синхронізації
...
mapper-copy-shared-intro = Створює вашу власну редаговану копію «{ $name }» точно такою, якою ви її зараз бачите. Те, до чого вам не надано доступ, не копіюється.
...
mapper-copy-atlas-offer = Ця мапа належить до атласу, який ви бачите. Ви можете відгалузити весь атлас — усі елементи, які ви дозволяєте копіювати, будуть додані. З'єднання між цими елементами буде переспрямовано на ваші копії.
...
mapper-can-edit-area = Може редагувати (спільне редагування ВАШОЇ канонічної мапи)
mapper-can-edit-folder = Може редагувати (спільне редагування ВАШИХ мап у цій теці)
mapper-can-reshare = Може повторно надавати доступ (може передати доступ на читання на один рівень углиб)
mapper-can-copy-area = Може копіювати (назавжди зберігає власне відгалуження й може його поширювати, незалежно від повторного надання доступу)
mapper-can-copy-folder = Може копіювати (назавжди зберігає власне відгалуження й може його поширювати)
mapper-include-secrets-area = Додати секрети
mapper-secrets-no-atlas = Секрети ніколи не додаються під час надання доступу до всього атласу — щоб їх додати, надайте доступ безпосередньо до області.
mapper-secrets-owner-only = Лише власник мапи може надавати доступ до її секретів.
mapper-make-admin-area = Надати права адміністратора — перейменування, видалення, переміщення, керування доступом і розкриття секретів (усе, крім передачі власності та призначення адміністраторів)
...
mapper-revoke-atlas-warning = Цей дозвіл охоплює весь атлас — його відкликання припиняє доступ до кожної області в атласі.
...
mapper-make-admin-folder = Надати права адміністратора — повний контроль над цією текою та її мапами (усе, крім передачі власності та призначення адміністраторів)
...
manifest-host-format-help = ім'я хоста або ім'я хоста:порт.
...
editor-priority-order-help = Вищий запускається першим; за однакового пріоритету зберігається порядок реєстрації.
...
package-direct-and-required = Його встановлено безпосередньо, і { $packages } також від нього залежить. Залишається ввімкненим, доки нічого його не потребує.
...
package-edit-copy-help = Створіть редаговану локальну копію цього пакунка.
...
package-save-before-publish = Збережіть зміни в маніфесті перед публікацією.
...
permission-can-sessions = Взаємодіяти з іншими відкритими сесіями
...
permission-can-panes = Створювати панелі виводу сесії та спрямовувати до них рядки гри
...
permission-cannot-display = взаємодіяти з терміналом гри (приховувати, стилізувати, вставляти чи замінювати текст; ні бачити поточний рядок)
...
package-fork-took-over = Ви редагуєте копію з назвою «{ $name }» — вона тепер активна, а оригінальну встановлену копію видалено.
package-fork-took-over-toast = Тепер ви редагуєте { $name } (встановлену копію видалено).
package-fork-mirrored = Ви редагуєте копію з назвою «{ $name }» — ваша локальна копія тепер активна.
package-fork-mirrored-toast = Тепер ви редагуєте { $name }.
package-fork-inactive = Ви редагуєте копію з назвою «{ $name }» — залишена вимкненою для читання; увімкніть її, щоб запустити.
...
runtime-package-duplicate-versions = [package] попередження: { $name } завантажено в { $count } співіснуючих версіях ({ $versions }) — побічні ефекти можуть конфліктувати; розгляньте розгалуження або закріплення
