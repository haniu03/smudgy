...
function boundedCoords(x: number, y: number, z: number): RoomFix["coords"] {
 ...
 *  behavior). */
let queue: Promise<void> = mapper.importAreasIfAbsent([]).then(
    () => {},
    () => {},
);
...
function takeMovement(fix: RoomFix, idDir: string | null, fixAt: number): string | null {
    ...
    const eligible = moveQueue.length > 0 && moveQueue[0].at <= fixAt;
    if (idDir) {
        if (eligible && moveQueue[0].dir === idDir) moveQueue.shift();
        return idDir;
    }
...
gmcp.onWrite("Room.WrongDir", (value: unknown) => {
...
function placement(areaId: AreaId, fix: RoomFix, direction: string | null): { x: number; y: number; level: number } {
    ...
    if (fix.coords) {
        ...
        let x = fix.coords.x * GRID;
        const y = fix.coords.y * GRID;
        const level = Math.round(fix.coords.z);
...
async function relinkExit(areaId: AreaId, room: RoomNumber, exit: Exit, toAreaId: AreaId, toRoomNumber: RoomNumber) {
...
async function reconcileKnownRoom(room: Room, fix: RoomFix) {
    ...
    for (const [dir, dest] of Object.entries(fix.exits)) {
        const existing = exitFor(room, dir);
        if (!existing) {
            await linkOrStub(areaId, roomNumber, dir, dest);
            continue;
        }
        if (existing.to_room_number !== null || dest === null || exitIsPending(existing.id)) {
            continue;
        }
        const destRoom = mapper.findRoomByExternalId(dest);
        if (destRoom) {
            await relinkExit(areaId, roomNumber, existing, destRoom.area_id, destRoom.room_number);
        } else {
            const waiters = pendingLinks.get(dest) ?? [];
            waiters.push({ areaId, room: roomNumber, exitId: existing.id, dir });
            pendingLinks.set(dest, waiters);
        }
    }
    if (pruneStale) {
...
// Fixes are timestamped at ARRIVAL (the watch delivery), not at processing: handleFix
// may run later (the maps-loaded barrier, a creation burst), and movement attribution
// must compare against when the server actually reported the room.
gmcp.watch("Room.Info", (info: unknown) => {
    const at = Date.now();
    enqueue(() => handleFix(adaptGmcp(info), at));
});
